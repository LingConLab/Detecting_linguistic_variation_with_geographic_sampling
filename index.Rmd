...
full_dataset %>%
  ggplot(aes(H, discovery_fraction*n_categories, color = factor(n_categories), linetype = type))+
  #geom_point()+
#  geom_smooth(se = FALSE, method = "glm", method.args = list(family = "binomial"))+
  geom_smooth()+
  facet_grid(location~n_categories, scales = "free_y")+
  scale_linetype_manual("", values = c(1, 2, 4))
ggsave("images/discovery_fracton_by_H.png")
```
